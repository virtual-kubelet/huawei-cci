...
import (
	...
	"github.com/virtual-kubelet/huawei-cci"
...
func runRootCommand(ctx context.Context, c Opts) error {
	...
	if c.Provider != "" && c.Provider != "huawei" {
		return errdefs.InvalidInputf("provider %q is not supported: only the huawei provider is supported", c.Provider)
	}

	p, err := huawei.NewCCIProvider(
